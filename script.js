...
function startSearchBubbleAnimation(startNode, endNode, maxJump) {
    ...
    searchBubble.scale.set(0.1, 0.1, 0.1); // Start small
    searchBubble.visible = true;

    const distance = startVec.distanceTo(endVec);
    const travelDuration = Math.max(1, distance / 100); // Adjust speed based on distance

    // Animation for the bubble traveling from start to end
    ...
    // Pulsing animation for the bubble's scale
    gsap.to(searchBubble.scale, {
        x: maxJump * 0.5, // Make the bubble size representative of the jump range
        y: maxJump * 0.5,
        z: maxJump * 0.5,
        duration: 0.7,
